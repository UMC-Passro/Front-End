...
사용자가 전달 요청 등록, 운송 가능 경로 확인, 매칭, 전달 진행 상태 확인 등의 핵심 기능을 편리하게 이용할 수 있도록 모바일 환경에 최적화된 웹 화면을 개발합니다.
...
│   ├── deliveryApi.ts     # 발송자 전달 API
│   ├── matchingApi.ts     # 전달자 매칭 및 상태 변경 API
...
3. 발송자는 추천된 전달자 후보를 확인하고 매칭 요청을 보냅니다.
4. 전달자가 요청을 수락하면 매칭이 확정되고 발송자에게 알림이 전달됩니다.
5. 전달자는 픽업, 이동 중, 전달 완료 등 전달 상태를 단계별로 업데이트합니다.
6. 전달 완료가 확인되면 포인트가 자동 정산됩니다.
7. 전달 완료 후 발송자는 리뷰와 평점을 남길 수 있습니다.
